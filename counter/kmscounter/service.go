...
// KMS counts the API operations made to AWS Key Management Service.
type KMS struct {
	svc    kmsiface.KMSAPI
	counts sync.Map
}

var _ kmsiface.KMSAPI = &KMS{}

// New creates a new instance of the KMS counter.
func New(svc kmsiface.KMSAPI) *KMS {
	return &KMS{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *KMS) Counters() map[string]int64 {
...
func (c *KMS) incViaRequestOption(op string) request.Option {
