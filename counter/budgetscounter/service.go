...
// Budgets counts the API operations made to AWS Budgets.
type Budgets struct {
	svc    budgetsiface.BudgetsAPI
	counts sync.Map
}

var _ budgetsiface.BudgetsAPI = &Budgets{}

// New creates a new instance of the Budgets counter.
func New(svc budgetsiface.BudgetsAPI) *Budgets {
	return &Budgets{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *Budgets) Counters() map[string]int64 {
...
func (c *Budgets) incViaRequestOption(op string) request.Option {
