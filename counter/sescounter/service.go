...
// SES counts the API operations made to Amazon Simple Email Service.
type SES struct {
	svc    sesiface.SESAPI
	counts sync.Map
}

var _ sesiface.SESAPI = &SES{}

// New creates a new instance of the SES counter.
func New(svc sesiface.SESAPI) *SES {
	return &SES{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *SES) Counters() map[string]int64 {
...
func (c *SES) incViaRequestOption(op string) request.Option {
