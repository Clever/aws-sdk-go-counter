...
// Lambda counts the API operations made to AWS Lambda.
type Lambda struct {
	svc    lambdaiface.LambdaAPI
	counts sync.Map
}

var _ lambdaiface.LambdaAPI = &Lambda{}

// New creates a new instance of the Lambda counter.
func New(svc lambdaiface.LambdaAPI) *Lambda {
	return &Lambda{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *Lambda) Counters() map[string]int64 {
...
func (c *Lambda) incViaRequestOption(op string) request.Option {
