...
// CloudHSM counts the API operations made to Amazon CloudHSM.
type CloudHSM struct {
	svc    cloudhsmiface.CloudHSMAPI
	counts sync.Map
}

var _ cloudhsmiface.CloudHSMAPI = &CloudHSM{}

// New creates a new instance of the CloudHSM counter.
func New(svc cloudhsmiface.CloudHSMAPI) *CloudHSM {
	return &CloudHSM{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *CloudHSM) Counters() map[string]int64 {
...
func (c *CloudHSM) incViaRequestOption(op string) request.Option {
