...
// Pricing counts the API operations made to AWS Price List Service.
type Pricing struct {
	svc    pricingiface.PricingAPI
	counts sync.Map
}

var _ pricingiface.PricingAPI = &Pricing{}

// New creates a new instance of the Pricing counter.
func New(svc pricingiface.PricingAPI) *Pricing {
	return &Pricing{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *Pricing) Counters() map[string]int64 {
...
func (c *Pricing) incViaRequestOption(op string) request.Option {
