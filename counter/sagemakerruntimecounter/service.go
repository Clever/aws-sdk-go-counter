...
// SageMakerRuntime counts the API operations made to Amazon SageMaker Runtime.
type SageMakerRuntime struct {
	svc    sagemakerruntimeiface.SageMakerRuntimeAPI
	counts sync.Map
}

var _ sagemakerruntimeiface.SageMakerRuntimeAPI = &SageMakerRuntime{}

// New creates a new instance of the SageMakerRuntime counter.
func New(svc sagemakerruntimeiface.SageMakerRuntimeAPI) *SageMakerRuntime {
	return &SageMakerRuntime{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *SageMakerRuntime) Counters() map[string]int64 {
...
func (c *SageMakerRuntime) incViaRequestOption(op string) request.Option {
