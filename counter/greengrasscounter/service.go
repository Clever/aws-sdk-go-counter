...
// Greengrass counts the API operations made to AWS Greengrass.
type Greengrass struct {
	svc    greengrassiface.GreengrassAPI
	counts sync.Map
}

var _ greengrassiface.GreengrassAPI = &Greengrass{}

// New creates a new instance of the Greengrass counter.
func New(svc greengrassiface.GreengrassAPI) *Greengrass {
	return &Greengrass{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *Greengrass) Counters() map[string]int64 {
...
func (c *Greengrass) incViaRequestOption(op string) request.Option {
