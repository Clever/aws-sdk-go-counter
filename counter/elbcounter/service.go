...
// ELB counts the API operations made to Elastic Load Balancing.
type ELB struct {
	svc    elbiface.ELBAPI
	counts sync.Map
}

var _ elbiface.ELBAPI = &ELB{}

// New creates a new instance of the ELB counter.
func New(svc elbiface.ELBAPI) *ELB {
	return &ELB{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *ELB) Counters() map[string]int64 {
...
func (c *ELB) incViaRequestOption(op string) request.Option {
