...
// StorageGateway counts the API operations made to AWS Storage Gateway.
type StorageGateway struct {
	svc    storagegatewayiface.StorageGatewayAPI
	counts sync.Map
}

var _ storagegatewayiface.StorageGatewayAPI = &StorageGateway{}

// New creates a new instance of the StorageGateway counter.
func New(svc storagegatewayiface.StorageGatewayAPI) *StorageGateway {
	return &StorageGateway{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *StorageGateway) Counters() map[string]int64 {
...
func (c *StorageGateway) incViaRequestOption(op string) request.Option {
