...
// WAF counts the API operations made to AWS WAF.
type WAF struct {
	svc    wafiface.WAFAPI
	counts sync.Map
}

var _ wafiface.WAFAPI = &WAF{}

// New creates a new instance of the WAF counter.
func New(svc wafiface.WAFAPI) *WAF {
	return &WAF{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *WAF) Counters() map[string]int64 {
...
func (c *WAF) incViaRequestOption(op string) request.Option {
