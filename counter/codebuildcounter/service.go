...
// CodeBuild counts the API operations made to AWS CodeBuild.
type CodeBuild struct {
	svc    codebuildiface.CodeBuildAPI
	counts sync.Map
}

var _ codebuildiface.CodeBuildAPI = &CodeBuild{}

// New creates a new instance of the CodeBuild counter.
func New(svc codebuildiface.CodeBuildAPI) *CodeBuild {
	return &CodeBuild{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *CodeBuild) Counters() map[string]int64 {
...
func (c *CodeBuild) incViaRequestOption(op string) request.Option {
