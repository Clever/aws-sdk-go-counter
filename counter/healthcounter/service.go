...
// Health counts the API operations made to AWS Health APIs and Notifications.
type Health struct {
	svc    healthiface.HealthAPI
	counts sync.Map
}

var _ healthiface.HealthAPI = &Health{}

// New creates a new instance of the Health counter.
func New(svc healthiface.HealthAPI) *Health {
	return &Health{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *Health) Counters() map[string]int64 {
...
func (c *Health) incViaRequestOption(op string) request.Option {
