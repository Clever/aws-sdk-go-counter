...
// APIGateway counts the API operations made to Amazon API Gateway.
type APIGateway struct {
	svc    apigatewayiface.APIGatewayAPI
	counts sync.Map
}

var _ apigatewayiface.APIGatewayAPI = &APIGateway{}

// New creates a new instance of the APIGateway counter.
func New(svc apigatewayiface.APIGatewayAPI) *APIGateway {
	return &APIGateway{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *APIGateway) Counters() map[string]int64 {
...
func (c *APIGateway) incViaRequestOption(op string) request.Option {
