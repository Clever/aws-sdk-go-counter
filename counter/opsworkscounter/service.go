...
// OpsWorks counts the API operations made to AWS OpsWorks.
type OpsWorks struct {
	svc    opsworksiface.OpsWorksAPI
	counts sync.Map
}

var _ opsworksiface.OpsWorksAPI = &OpsWorks{}

// New creates a new instance of the OpsWorks counter.
func New(svc opsworksiface.OpsWorksAPI) *OpsWorks {
	return &OpsWorks{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *OpsWorks) Counters() map[string]int64 {
...
func (c *OpsWorks) incViaRequestOption(op string) request.Option {
