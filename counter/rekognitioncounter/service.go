...
// Rekognition counts the API operations made to Amazon Rekognition.
type Rekognition struct {
	svc    rekognitioniface.RekognitionAPI
	counts sync.Map
}

var _ rekognitioniface.RekognitionAPI = &Rekognition{}

// New creates a new instance of the Rekognition counter.
func New(svc rekognitioniface.RekognitionAPI) *Rekognition {
	return &Rekognition{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *Rekognition) Counters() map[string]int64 {
...
func (c *Rekognition) incViaRequestOption(op string) request.Option {
