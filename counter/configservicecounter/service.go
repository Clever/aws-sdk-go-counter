...
// ConfigService counts the API operations made to AWS Config.
type ConfigService struct {
	svc    configserviceiface.ConfigServiceAPI
	counts sync.Map
}

var _ configserviceiface.ConfigServiceAPI = &ConfigService{}

// New creates a new instance of the ConfigService counter.
func New(svc configserviceiface.ConfigServiceAPI) *ConfigService {
	return &ConfigService{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *ConfigService) Counters() map[string]int64 {
...
func (c *ConfigService) incViaRequestOption(op string) request.Option {
