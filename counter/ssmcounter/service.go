...
// SSM counts the API operations made to Amazon Simple Systems Manager (SSM).
type SSM struct {
	svc    ssmiface.SSMAPI
	counts sync.Map
}

var _ ssmiface.SSMAPI = &SSM{}

// New creates a new instance of the SSM counter.
func New(svc ssmiface.SSMAPI) *SSM {
	return &SSM{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *SSM) Counters() map[string]int64 {
...
func (c *SSM) incViaRequestOption(op string) request.Option {
