...
// KinesisAnalytics counts the API operations made to Amazon Kinesis Analytics.
type KinesisAnalytics struct {
	svc    kinesisanalyticsiface.KinesisAnalyticsAPI
	counts sync.Map
}

var _ kinesisanalyticsiface.KinesisAnalyticsAPI = &KinesisAnalytics{}

// New creates a new instance of the KinesisAnalytics counter.
func New(svc kinesisanalyticsiface.KinesisAnalyticsAPI) *KinesisAnalytics {
	return &KinesisAnalytics{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *KinesisAnalytics) Counters() map[string]int64 {
...
func (c *KinesisAnalytics) incViaRequestOption(op string) request.Option {
