...
// Athena counts the API operations made to Amazon Athena.
type Athena struct {
	svc    athenaiface.AthenaAPI
	counts sync.Map
}

var _ athenaiface.AthenaAPI = &Athena{}

// New creates a new instance of the Athena counter.
func New(svc athenaiface.AthenaAPI) *Athena {
	return &Athena{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *Athena) Counters() map[string]int64 {
...
func (c *Athena) incViaRequestOption(op string) request.Option {
