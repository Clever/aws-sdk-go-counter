...
// MQ counts the API operations made to AmazonMQ.
type MQ struct {
	svc    mqiface.MQAPI
	counts sync.Map
}

var _ mqiface.MQAPI = &MQ{}

// New creates a new instance of the MQ counter.
func New(svc mqiface.MQAPI) *MQ {
	return &MQ{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *MQ) Counters() map[string]int64 {
...
func (c *MQ) incViaRequestOption(op string) request.Option {
