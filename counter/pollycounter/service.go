...
// Polly counts the API operations made to Amazon Polly.
type Polly struct {
	svc    pollyiface.PollyAPI
	counts sync.Map
}

var _ pollyiface.PollyAPI = &Polly{}

// New creates a new instance of the Polly counter.
func New(svc pollyiface.PollyAPI) *Polly {
	return &Polly{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *Polly) Counters() map[string]int64 {
...
func (c *Polly) incViaRequestOption(op string) request.Option {
