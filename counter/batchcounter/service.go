...
// Batch counts the API operations made to AWS Batch.
type Batch struct {
	svc    batchiface.BatchAPI
	counts sync.Map
}

var _ batchiface.BatchAPI = &Batch{}

// New creates a new instance of the Batch counter.
func New(svc batchiface.BatchAPI) *Batch {
	return &Batch{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *Batch) Counters() map[string]int64 {
...
func (c *Batch) incViaRequestOption(op string) request.Option {
