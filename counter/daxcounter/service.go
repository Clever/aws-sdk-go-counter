...
// DAX counts the API operations made to Amazon DynamoDB Accelerator (DAX).
type DAX struct {
	svc    daxiface.DAXAPI
	counts sync.Map
}

var _ daxiface.DAXAPI = &DAX{}

// New creates a new instance of the DAX counter.
func New(svc daxiface.DAXAPI) *DAX {
	return &DAX{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *DAX) Counters() map[string]int64 {
...
func (c *DAX) incViaRequestOption(op string) request.Option {
