...
// CognitoSync counts the API operations made to Amazon Cognito Sync.
type CognitoSync struct {
	svc    cognitosynciface.CognitoSyncAPI
	counts sync.Map
}

var _ cognitosynciface.CognitoSyncAPI = &CognitoSync{}

// New creates a new instance of the CognitoSync counter.
func New(svc cognitosynciface.CognitoSyncAPI) *CognitoSync {
	return &CognitoSync{svc: svc}
}

// Counters returns a snapshot of current counter values for each API operation.
func (c *CognitoSync) Counters() map[string]int64 {
...
func (c *CognitoSync) incViaRequestOption(op string) request.Option {
